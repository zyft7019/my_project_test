 ...
 (function () {
         ...
         (function () {
             ...
             /**
1          * 判断目标参数是否为function或Function实例
1          * @name baidu.lang.isFunction
1          * @function
1          * @grammar baidu.lang.isFunction(source)
1          * @param {Any} source 目标参数
1          * @returns {boolean} 类型判断结果
1          */
             ...
             /**
1          * 重载了默认的toString方法，使得返回信息更加准确一些。
1          * @return {string} 对象的String表示形式
1          */
...
1          * 自定义的事件对象。
1          * @function
1          * @name baidu.lang.Event
1          * @grammar baidu.lang.Event(type[, target])
1          * @param {string} type	 事件类型名称。为了方便区分事件和一个普通的方法，事件类型名称必须以"on"(小写)开头。
1          * @param {Object} [target]触发事件的对象
1          * @meta standard
1          * @remark 引入该模块，会自动为Class引入3个事件扩展方法：addEventListener、removeEventListener和dispatchEvent。
1          * @see baidu.lang.Class
1          */
             ...
             /**
1          * 注册对象的事件监听器。引入baidu.lang.Event后，Class的子类实例才会获得该方法。
1          * @grammar obj.addEventListener(type, handler[, key])
1          * @param 	{string}   type         自定义事件的名称
1          * @param 	{Function} handler      自定义事件被触发时应该调用的回调函数
1          * @param 	{string}   [key]		为事件监听函数指定的名称，可在移除时使用。如果不提供，方法会默认为它生成一个全局唯一的key。
1          * @remark 	事件类型区分大小写。如果自定义事件名称不是以小写"on"开头，该方法会给它加上"on"再进行判断，即"click"和"onclick"会被认为是同一种事件。 
1          */
             ...
             /**
1          * 移除对象的事件监听器。引入baidu.lang.Event后，Class的子类实例才会获得该方法。
1          * @grammar obj.removeEventListener(type, handler)
1          * @param {string}   type     事件类型
1          * @param {Function|string} handler  要移除的事件监听函数或者监听函数的key
1          * @remark 	如果第二个参数handler没有被绑定到对应的自定义事件中，什么也不做。
1          */
             ...
             /**
1          * 派发自定义事件，使得绑定到自定义事件上面的函数都会被执行。引入baidu.lang.Event后，Class的子类实例才会获得该方法。
1          * @grammar obj.dispatchEvent(event, options)
1          * @param {baidu.lang.Event|String} event 	Event对象，或事件名称(1.1.1起支持)
1          * @param {Object} options 扩展参数,所含属性键值会扩展到Event对象上(1.2起支持)
1          * @remark 处理会调用通过addEventListenr绑定的自定义事件回调函数之外，还会调用直接绑定到对象上面的自定义事件。
1          * 例如：<br>
1          * myobj.onMyEvent = function(){}<br>
2          * myobj.addEventListener("onMyEvent", function(){});
2          */
             ...
             /**
2          * 为类型构造器建立继承关系
2          * @name baidu.lang.inherits
2          * @function
2          * @grammar baidu.lang.inherits(subClass, superClass[, className])
2          * @param {Function} subClass 子类构造器
2          * @param {Function} superClass 父类构造器
2          * @param {string} className 类名标识
2          * @remark 使subClass继承superClass的prototype，
2          * 因此subClass的实例能够使用superClass的prototype中定义的所有属性和方法。<br>
2          * 这个函数实际上是建立了subClass和superClass的原型链集成，并对subClass进行了constructor修正。<br>
2          * <strong>注意：如果要继承构造函数，需要在subClass里面call一下，具体见下面的demo例子</strong>
2          * @shortcut inherits
2          * @meta standard
2          * @see baidu.lang.Class
2          */
             ...
             /**
2          * @ignore
2          * @namespace baidu.dom 操作dom的方法。
2          */
             baidu.dom = baidu.dom || {};
...
2          * @name baidu.dom.g
2          * @function
2          * @grammar baidu.dom.g(id)
2          * @param {string|HTMLElement} id 元素的id或DOM元素
2          * @meta standard
2          *             
2          * @returns {HTMLElement|null} 获取的元素，查找不到时返回null,如果参数不合法，直接返回参数
2          */
             ...
             /**
2          * @ignore
2          * @namespace baidu.browser 判断浏览器类型和特性的属性。
2          */
...
2              * 判断是否为ie浏览器
2              * @property ie ie版本号
2              * @grammar baidu.browser.ie
2              * @meta standard
2              * @shortcut ie
2              * @see baidu.browser.firefox,baidu.browser.safari,baidu.browser.opera,baidu.browser.chrome,baidu.browser.maxthon 
2              */
             ...
             /**
3          * 提供给setAttr与getAttr方法作名称转换使用
3          * ie6,7下class要转换成className
3          * @meta standard
3          */
             ...
             /**
3          * 获取目标元素的属性值
3          * @name baidu.dom.getAttr
3          * @function
3          * @grammar baidu.dom.getAttr(element, key)
3          * @param {HTMLElement|string} element 目标元素或目标元素的id
3          * @param {string} key 要获取的attribute键名
3          * @shortcut getAttr
3          * @meta standard
3          * @see baidu.dom.setAttr,baidu.dom.setAttrs
3          *             
3          * @returns {string|null} 目标元素的attribute值，获取不到时返回null
3          */
             ...
             /**
3          * @ignore
3          * @namespace baidu.event 屏蔽浏览器差异性的事件封装。
3          * @property target 	事件的触发元素
3          * @property pageX 		鼠标事件的鼠标x坐标
3          * @property pageY 		鼠标事件的鼠标y坐标
3          * @property keyCode 	键盘事件的键值
3          */
...
3          */
             ...
             /**
3          * 为目标元素添加事件监听器
3          * @name baidu.event.on
3          * @function
3          * @grammar baidu.event.on(element, type, listener)
3          * @param {HTMLElement|string|window} element 目标元素或目标元素id
3          * @param {string} type 事件类型
3          * @param {Function} listener 需要添加的监听器
3          * @remark
3          *  1. 不支持跨浏览器的鼠标滚轮事件监听器添加<br>
3          *  2. 改方法不为监听器灌入事件对象，以防止跨iframe事件挂载的事件对象获取失败            
3          * @shortcut on
3          * @meta standard
3          * @see baidu.event.un
3          *             
3          * @returns {HTMLElement|window} 目标元素
3          */
...
4        */
         ...
         /** 
4      * @exports CityList as BMapLib.CityList 
4      */
...
4          * CityList类的构造函数
4          * @class 城市列表类，
4          * 实例化该类后，可以帮助用户直接生成城市列表，
5          * 也可以通过接口获取城市数据。
5          * 
5          * @constructor
5          * @param {Json Object} opts 可选的输入参数，非必填项。可输入选项包括：<br />
5          * {"<b>container</b>" : {String|HTMLElement} 需要提供界面方式展现的容器。如果此参数为空，则不提供界面方式，也没有cityclick的事件派发
5          * <br />"<b>map</b>" : {BMap} 实例化的map对象，如果传入此参数，则用户点击界面中的城市时，可以直接帮助用户定位到地图的相关城市位置}
5          *
5          * @example <b>参考示例：</b><br />
5          * var myCityListObject = new BMapLib.CityList({container : "container"});
5          */
